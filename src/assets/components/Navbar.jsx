...
const Navbar = ({ navRef, scrolled }) => {
  ...
  const menuOpen = () => {
    ...
    setisbgBlack(newMenuState);
    setIsMenuOpen(newMenuState);
    setCloseMenu(newMenuState);
  };
  ...
  const cartOpen = () => {
    ...
    setisbgBlack(newCartState);
    setIsCartOpen(newCartState);
    setCloseCart(newCartState);
  };

  const cart1 = useRef(null);
  const cart2 = useRef(null);
  const empty1 = useRef(null);
  const empty2 = useRef(null);
  const scrollAnimation = () => {
    gsap.to(empty1.current, {
      xPercent: 100,
      duration: 20,
      repeat: -1,
      ease: "linear",
    ...
    gsap.to(cart1.current, {
      xPercent: -100,
      duration: 20,
      ease: "none",
      repeat: -1,
    });
    gsap.to(cart2.current, {
      xPercent: -100,
      duration: 20,
      ease: "none",
      repeat: -1,
  ...
  return (
    <div
      ref={navRef}
      className={`w-full  py-5  z-50 text- px-3 md:px-5 fixed top-0  border-gray-500   ${
    ...
    >
      <div className="flex md:items-center justify-between">
        ...
        <div className="flex flex-col gap-y-5 relative">
          <div
            className={`w-[80px] lg:w-auto transition-all duration-200 transform -translate-y-[103px] absolute ${
              scrolled && "translate-y-[0px]"
            }`}
          >
            <svg
              className=""
          ...
          <div
            className={`w-[80px] lg:w-auto transition-all duration-200   transform -translate-y-[0px] ${
              scrolled && "-translate-y-[206px]"
            } `}
          >
            <svg
              width="106"
        ...
        <div>
          <ul className="flex items-start gap-10 lg:gap-20 font-medium ">
            {navLinks.map((itme, i) => (
              <div
                key={i}
                className={`relative hidden lg:block  transform ${
                  scrolled
            ...
            <div
              className={`flex font-medium  rounded-full min-h-[10px] items-center py-2  px-5  ${
                isBgBlack ? "bg-black" : "bg-white"
              } `}
            >
              {/* Menu */}
              <li className="overflow-hidden">
                <button onClick={menuOpen} className="cursor-pointer flex">
                  <CiMenuFries
                    size={18}
                ...
                {/* MENU DropDown */}
                <div
                  className={`absolute left-[0%] top-0 w-[100%] transition-all duration-500 overflow-hidden overflow-y-auto   ${
                    isMenuOpen ? "h-screen z-[-999]" : "h-0"
                  }  bg-black  `}
                >
                  <div className="flex flex-col-reverse lg:flex-row relative justify-between h-full pt-20 gap-10 px-5  ">
                        ...
                        <div>
                          ...
                          <ul class="">
                            <li>
                              <a
                                href="#"
                                class="hover:text-gray-500 transition-all duration-200 text-xs md:text-sm"
                              >
                                Facebook
                              </a>
                            </li>
                    ...
                    <div className="flex flex-col h-full">
                      {menu.map((item, i) => (
                        <div className="hover:scale-110 transition-all duration-300 transform hover:-translate-x-10 ">
                          <h3 className="  text-3xl lg:text-5xl 2xl:text-7xl font-extrabold mb-1 text-end cursor-pointer ">
                            {item}
                          </h3>
                        </div>
                      ))}
              ...
              <li>
                ...
                >
                  ...
                  <MdClose
                    ...
                    className={`transition-all duration-300 ${
                      closeCart
                        ? "opacity-100 scale-100 translate-x-5"
                        : "opacity-0 scale-0 translate-x-0 "
                    }`}
                  />
                </button>
                ...
                <div
                  className={`absolute left-[0%] top-0 w-[100%] transition-all duration-500 overflow-hidden ${
                    isCartOpen ? "  h-fit  z-[-999]" : "h-0 z-0"
                  }  bg-black  `}
                >
                  <div className=" pt-40 w-full  relative ">
                    {" "}
                    <div className=" w-full h-full">
                      <h4 className="text-center text-xl md:text-3xl capitalize font-medium mt-5 md:my-10">
                        Your Cart is Empty
                      </h4>
                      <h3 className="text-center text-md md:text-lg capitalize font-medium border-2 w-fit px-10 md:px-20 mx-auto transition-all duration-200 cursor-pointer border-white hover:bg-white hover:text-black rounded-full my-10 py-5 ">
                        see all the good things
                      </h3>
                    </div>
                    <div className="w-full  py-2 md:py-5 border border-x-0 flex gap-10  ">
                      <div ref={cart1} className="flex items-center gap-10  ">
                        <h3 className="text-2xl md:text-4xl lg:text-8xl uppercase font-bold">
                          cart
                        </h3>
                        <h3 className="text-2xl md:text-4xl lg:text-8xl uppercase font-bold">
                          cart
                      ...
                      <div ref={cart2} className="flex items-center gap-10  ">
                        <h3 className="text-2xl md:text-4xl lg:text-8xl uppercase font-bold">
                          cart
                        </h3>
                        <h3 className="text-2xl md:text-4xl lg:text-8xl uppercase font-bold">
                    ...
                    <div className="bg-black max-w-dvw  py-2 md:py-5 border-2 border-x-0 flex  overflow-hidden gap-20">
                      <div
                        ref={empty1}
                        className="flex items-center shrink text-wrap gap-10 min-w-[100%] overflow-hidden transform -translate-x-[100%] "
                      >
                        <h3 className=" text-2xl md:text-4xl lg:text-8xl uppercase font-bold">
                          empty
                        </h3>
